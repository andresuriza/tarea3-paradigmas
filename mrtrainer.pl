...
word(U1) --> [K],{lc(K,K1)},!,alphanums(U2),{nombre(U1,[K1|U2])}.
word(nb(N)) --> [K],{digit(K)},!,digits(U),{nombre(N,[K|U])}.
word(V) --> [K],{nombre(V,[K])}.
...
oracion(s(X, Y, Z)) --> pronombre_suj(X), indicative_verb(Y),
                         adjetivo(Z).
...
abstract_noun(abs_noun(nombre)) --> [nombre].

special_noun(sp_noun(justin)) --> [justin].
special_noun(sp_noun(frank)) --> [frank].

subject_phrase(sp(X)) --> pronombre_suj(X).
...
pronombre_suj(spn(yo)) --> [yo].
pronombre_suj(spn(nosotros)) --> [nosotros].
pronombre_suj(spn(usted)) --> [usted].
pronombre_suj(spn(ellos)) --> [ellos].
pronombre_suj(spn(el)) --> [el].
pronombre_suj(spn(ella)) --> [ella].
pronombre_suj(spn(eso)) --> [eso].
pronombre_suj(spn(quien)) --> [quien].
...
question(q(why,do,S)) --> [why, do], oracion(S).
question(q(do,S)) --> [do], oracion(S).

question(q(X, Y, Z)) --> adverb(X), indicative_verb(Y), pronombre_suj(Z).
...
mapping_noun(nombre, frank).
mapping_noun(frank, nombre).
...
respuestas_db(random_s, [
	['Disculpa, no entend� lo que dices'],
	['�Podr�as repetir eso?'],
